...
# file_source="##源文件##"
# file_target="###目标文件###"
from  Hier_lstm_att_model_2018_1_24_test import RNN_Model
...
def saveTestResult(words,length_array_eachdoc_source):
    '''
    根据return_length_array_eachdoc_source整理出每一篇文章的单词个数
    '''
    filePath = 'data/gen_doc.txt'
    num_words_each_doc=[]                                                                                 #存放每一篇文章的单词个数
    for i,length_array_a_doc in enumerate(length_array_eachdoc_source):
        num=0
        for j,num_each_sen in enumerate(length_array_a_doc):
            num+=int(num_each_sen)
        num_words_each_doc.append(num)
    with open(filePath,mode='a') as fout:
        for gen_word_each,num_each_doc_source in zip(words,num_words_each_doc):
            end_index=math.floor(1.5*num_each_doc_source)
            words_limit=gen_word_each[:end_index]
            if FLAGS.vocabulary_size-1 in words_limit:
                end_index=words_limit.index(FLAGS.vocabulary_size-1)
            buffer=" ".join([str(word) for word in gen_word_each[:end_index] ])+"\n"
            fout.write(buffer)
...
def run_epoch(model,session,data_source,data_target):
    ...
    for source,target in zip(  data_helper_source_2018_1_20.batch_iter(data_source, FLAGS.batch_size), data_helper_target_2018_1_20.batch_iter(data_target,FLAGS.batch_size)):
        sen_mask, train_source_set, mask_train_source_set, length_array_eachdoc_source=source
        ...
        fetches = [model.result,model.words]
        result,first_words= session.run(fetches, feed_dict)
        saveTestResult(result, length_array_eachdoc_source)
